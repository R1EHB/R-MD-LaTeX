...
\begin{frame}{Setting up knitr}
    \url{https://www.rdocumentation.org/packages/knitr}

    Install knitr.

    If you are going to write Rnw documents (documents written in LaTeX with R code embedded in them), go to the menu $Tools~>~Options~>~Sweave$ and switch the default option for weaving (compiling) to knitr.
\end{frame}
...
In Rstudio, quickly insert chunks with the keyboard shortcut $Ctrl + Alt + I$. This works for both $.Rnw$ and $.Rmd$ code.
...
\begin{frame}[fragile]{Quick reporting in Markdown}
    ...
    \begin{exampleblock}{Stitching quick reports}
    \begin{lstlisting}
    library(knitr)
...
\begin{itemize}
    ...
    \item knitr documents source + output examples \url{https://yihui.name/knitr/demos/}
