...
\column{0.5\textwidth}
\vfill
\includegraphics[width=4.0cm]{markdown-examples.png}
\vfill
...
\begin{alertblock}{Limitations of Markdown}
  Markdown was primarily designed to be simple. \\ For more
  complicated typesetting, LaTeX may be preferred.
\end{alertblock}
\vfill
\end{frame}
...
% Put Erik and Emily's test examples here
...
Have you ever included a table in your LaTeX document? \\ In LaTeX, it would look like this...
\end{frame}
...
\begin{frame}[fragile]{Sweave}
    \vfill
    Sweave has been a prominent longstanding tool for dynamic documents since 2002.
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Extensions may become incompatible. Some packages are no longer synchronized.
...
First of all, knitr uses Rmarkdown, a set of intuitive human-readable
code to do the formatting. While LaTeX is by no means as complicated
as its reputation seems to suggest, Rmarkdown is actually easy. By
human-readable I mean that anyone who has never even heard of
Rmarkdown can understand what is happening to some extent.

Sweave is great for producing PDF, but that’s one of the biggest
drawbacks of LaTeX in the social sciences: while the PDF may look
good, they are not the format we need when collaborating with
Word-only colleagues, and with rare exceptions when submitting a
manuscript to journals. Knitr works very well with Pandoc, so creating
a Word document or an ODF is just as easy as creating a PDF. The other
day I had to submit a supplementary file as a *.doc file, even though
it’ll end up as a PDF on Dataverse or so. With knitr this didn’t take
long.

knitr supports R, Python, Ruby, Haskell, awk/gawk, sed, shell scripts,
Perl, SAS, TikZ, Graphviz, C++, and more.
...
Work in progress
    \begin{itemize}
        \item LaTeX (to produce Rnw documents) is more complicated than RMarkdown (Rmd), and documents rarely need to be produced as PDFs unless submitting a manuscript to journals.
    \end{itemize}
Check out \url{https://www.rdocumentation.org/packages/knitr} for a more info on installation, motivation, usage, and functions.
\end{frame}
...
\vfill
    ...
    \end{itemize}
\vfill
\begin{exampleblock}{Oh, so now the file isn't ``.TeX'' anymore?}
``Rnw'' stands for ``R no web''. \href{https://www.cs.tufts.edu/~nr/noweb/}{NoWeb} is an old, still active simple system for mixing code and narratives.
\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{R in Markdown}
\vfill
\begin{itemize}
...
\end{itemize}
\vfill
\begin{exampleblock}{Everybody gets a shortcut!}
In Rstudio, quickly insert code chunks with the keyboard shortcut $Ctrl + Alt + I$. This works for both $.Rnw$ and $.Rmd$ code.
(OS X: $Cmd + Option + I$)
\end{exampleblock}
\vfill
\end{frame}
...
\begin{frame}{R in Markdown \& LaTeX}
    ...
    \item[LaTeX] has more commands to learn, but always looks better---even with minimal coding. Plus, there is no way to reference figures in Markdown.
    \end{description}
    \vfill
    ...
    %\vfill
    %Also, \textbf{you can use them both at the same time}. Theoretically.
    % Maybe one day we will never be restricted by the physical space of an 8.5x11-inch piece of paper ever again, and typesetting a ``90-page'' paper will become a simple matter of making your report scroll linearly, but that day is not today.
...
%    This would be a slide for calling to pandoc to get an Rmd file into a LaTeX document. Once I get that to work.
%\end{frame}
...
%        \end{columns}
%    \end{frame}
\subsection{Semi-realistic Example of Reproducible Research}
\begin{frame}{Air Data Analysis}
  LaTeX/Markdown, knitr, and R are great for this:
  \begin{itemize}
  \item Scripting/Batch File
  \item Easy to revise
  \item History of revisions (especially when paired with git)
  \item Small change, big update
  \end{itemize}
\end{frame}
